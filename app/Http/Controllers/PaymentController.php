...
class PaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $totalParcels = $pdfData['total_parcels'];
            $parcelRowsCount = $pdfData['parcel_rows_count'];
    ...
    private function parsePdfFile($file)
    {
        try {
            ...
            // Extract Total Parcels
            $totalParcels = $this->extractTotalParcels($text);

            // Extract Parcel Rows Count
    ...
    /**
     * Extract Total Parcels from PDF text
     * The green highlighted value in transaction summary total row
     */
    private function extractTotalParcels($text)
    {
        // Look for Total row in transaction summary with parcels invoiced qty
        if (preg_match('/Total\s+\d+\s+(\d+)/i', $text, $matches)) {
    ...
    /**
     * Extract Parcel Rows Count from PDF text
     * Count rows in transaction summary where Parcels Invoiced Qty > 0
     */
    private function extractParcelRowsCount($text)
    {
        // Find transaction summary section
        if (preg_match('/Transaction\s+summary(.*?)(?:Fuel|Manual)/is', $text, $summaryMatch)) {
            $summary = $summaryMatch[1];
            $lines = explode("\n", $summary);
            $count = 0;

            foreach ($lines as $line) {
                // Match lines with date pattern and check if parcel qty > 0
                if (preg_match('/\d{4}-\d{2}-\d{2}.*?(\d+).*?(\d+)/i', $line, $matches)) {
                    // Check if second number (parcel qty) is > 0
                    if ((int)$matches[2] > 0) {
                        $count++;
                    }
                }
